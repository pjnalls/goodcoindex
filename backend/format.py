...
from backend.tinysearchengine.indexer import DocumentState
from backend.tokenizer import tokenize, clean_unicode

DOCUMENT_SOURCES = {
    DocumentState.FROM_GOOGLE: 'google',
    DocumentState.FROM_USER: 'user',
    DocumentState.FROM_WIKI: 'wikipedia',
    DocumentState.ORGANIC_APPROVED: 'goodcoindex',
...
def get_document_source(state: DocumentState):
    return DOCUMENT_SOURCES.get(state, 'goodcoindex')
...
def format_result_with_pattern(pattern, result):
    ...
    for content_type, content_raw in [('title', result.title), ('extract', result.extract)]:
        ...
        all_spans = [0] + sum((list(m.span())
                              for m in matches), []) + [len(content)]
        content_result = []
        for i in range(len(all_spans) - 1):
            ...
            content_result.append(
                {'value': content[start:end], 'is_bold': is_bold})
...
def get_query_regex(terms, is_complete, is_url):
    ...
    if is_complete:
        term_patterns = [
            rf'{word_sep}{re.escape(term)}{word_sep}' for term in terms]
    else:
